...
PeerSync.prototype.checkStatus = function() {
  ...
  console.log("PeerSync.prototype.checkStatus connected: " + this.connected + ", timer: ")
  console.log((!this.connected && this.timer))
  if (!this.connected && this.timer) {
    this.run();
  }
};

PeerSync.prototype.updateTrigger = function() {
	console.log("--------------- innerCheatTrigger enging ------------------------")
  innerCheatTrigger--;
};
...
PeerSync.prototype.run = function() {
  ...
  this.peerman.on('netDisconnected', function() {
	  console.log("PeerSync.prototype.run netDisconnected checked")
    self.connected = false;
    if (!self.timer) 
		self.timer = setInterval(self.checkStatus.bind(self), self.interval);
	else {
		
		if(innerCheatTrigger == 1){
			
			console.log("start check peers: " + self.peerman.peers.length);
			if(self.peerman.peers.length == 0)
				...
				self.peerdb.forEach(function(datum) {
					var peer = new Peer(datum.ipv4, datum.port);
					var isPresent = 0;
					for(var index = 0;index < self.peerman.peers.length; index++){
					  if(self.peerman.peers[index].toString() === peer.toString()){
						 isPresent = 1;
					  }
				    }
					if(isPresent == 0)
		...
		if(innerCheatTrigger == 0 ){
			innerCheatTrigger++;
			//var innerTimer = setInterval(self.updateTrigger.bind(self), self.interval);
			self.peerdb.forEach(function(datum) {
				var peer = new Peer(datum.ipv4, datum.port);
				self.peerman.addPeer(peer);
			});
